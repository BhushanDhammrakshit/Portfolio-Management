...
def _classify_sentiment(headline_lower: str) -> str:
    """Return 'bullish', 'bearish', or 'neutral' from keyword scoring."""
    bull = 0
    bear = 0
...
def fetch_and_parse_stock_news():
    ...
    for headline, pub_time in headlines:
        ...
        if not stock:
            continue
        sentiment = _classify_sentiment(lower)
        key = (stock, lower[:60])
        if key in seen:
            continue
        seen.add(key)
...
def show_stock_news():
    ...
    return render_template(
        ...
        title="News & Trends",
        last_updated=last_updated.replace(tzinfo=datetime.timezone.utc).astimezone(IST).strftime("%d %b %Y, %I:%M %p IST")
