...
def get_advanced_analytics():
    """Compute advanced portfolio metrics using historical data from yfinance."""
    ...
    try:
        ...
        if hasattr(close.columns, "levels"):
            pass  # multi-index already
        # Handle single vs multi-symbol
        if len(all_symbols) == 1:
            close = close.to_frame(name=all_symbols[0])
    except Exception as e:
        print(f"[advanced_analytics] yfinance download error: {e}")
        return jsonify(_fallback_metrics(stock_data, total_value))
