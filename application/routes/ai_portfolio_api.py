...
from application.services.ai_client import chat_with_usage, is_configured
from application.services.token_limiter import (
    add_usage,
    get_usage,
    remaining,
)
...
@ai_portfolio_api.route("/api/ai_portfolio_insight/usage", methods=["GET"])
def ai_portfolio_usage():
    ...
    return jsonify({
        "used": used,
        "limit": limit,  # None = unlimited (Elite)
...
def ai_portfolio_insight():
    ...
    # Enforce per-user monthly token limit before calling the AI.
    # ``limit is None`` means unlimited (Elite fair-use).
    used_before, limit = get_usage(user)
    if limit is not None and used_before >= limit:
        return jsonify({
            "error": "monthly_limit_reached",
            "message": (
                f"You’ve hit your monthly AI limit of {limit:,} tokens. "
                "It will reset on the 1st, or upgrade your plan for more."
            ),
            "used": used_before,
            "limit": limit,
            "remaining": 0,
    ...
    if not is_configured():
        return jsonify({
            ...
            "used": used_before,
            "limit": limit,
            "remaining": None if limit is None else max(0, limit - used_before),
        })
    ...
    # Cap max_tokens to whatever the user has left so we never exceed the
    # monthly limit on a single response. Elite (unlimited) gets the full 900.
    rem_budget = remaining(user)
    capped_max = 900 if rem_budget is None else min(900, max(1, rem_budget))

    content, err, usage = chat_with_usage(
        [
            {"role": "system", "content": PORTFOLIO_SYSTEM_PROMPT},
            {"role": "user", "content": prompt},
        ],
        temperature=0.6, max_tokens=capped_max,
    )

    spent = int(usage.get("total_tokens") or 0)
    if spent > 0:
        used_after, _ = add_usage(user, spent)
    else:
