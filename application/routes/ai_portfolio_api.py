...
def ai_portfolio_insight():
    ...
    # Enforce per-user daily token limit before calling the AI
    used_before, limit = get_usage(user)
    if used_before >= limit:
        return jsonify({
            "error": "daily_limit_reached",
            "message": (
                f"You\u2019ve hit your daily AI limit of {limit:,} tokens. "
                "It will reset automatically tomorrow."
            ),
            "used": used_before,
            "limit": limit,
            "remaining": 0,
    ...
    if not is_configured():
        return jsonify({
            ...
            "used": used_before,
            "limit": limit,
            "remaining": max(0, limit - used_before),
        })
    ...
    # daily limit on a single response.
    remaining_budget = max(1, remaining(user))
    capped_max = min(900, remaining_budget)
