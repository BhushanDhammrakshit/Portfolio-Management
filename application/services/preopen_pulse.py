...
# Canonical prior-day close anchor for NIFTY. The live pre-open feed
# (Yahoo / Dhan) occasionally swaps `prev_close` with the most recent
# indicative open, flipping the gap-% chart wildly; we anchor to the
# daily-history close (which is glitch-free) and cache it for the day.
_PREV_CLOSE_KEY_FMT = "preopen:prev_close:{date}"
_PREV_CLOSE_TTL = 12 * 3600
_PREV_CLOSE_TOL = 0.015  # reject provider prev_close >1.5% off the anchor
...
def _canonical_prev_close(day_key: str) -> Optional[float]:
    """Trusted prior-day close for NIFTY, anchored to the daily-history bar.

    Cached for the session because daily history rarely changes intraday.
    """
    cache_key = _PREV_CLOSE_KEY_FMT.format(date=day_key)
    cached = shared_cache.jget(cache_key)
    if isinstance(cached, (int, float)) and cached > 0:
        return float(cached)
    try:
        hist = market_data.get_history(NIFTY_INDEX, days=8, interval="1d")
        if hist is None or hist.empty or "Close" not in hist.columns:
            return None
        closes = hist["Close"].dropna()
        # Drop today's bar if the provider has rolled it forward.
        try:
            if closes.index[-1].strftime("%Y%m%d") >= day_key and len(closes) >= 2:
        ...
        if prev > 0:
            shared_cache.jset(cache_key, prev, ttl=_PREV_CLOSE_TTL)
            return prev
    except Exception as e:
        log.debug("canonical prev_close fetch failed: %s", e)
    return None


def _sanitize_series(rows: List[Dict[str, Any]],
                     canonical: Optional[float]) -> Tuple[List[Dict[str, Any]], bool]:
    """Replace bogus prev_close values written by glitched ticks."""
    if not canonical or canonical <= 0 or not rows:
        return rows, False
    changed = False
    for r in rows:
        ...
        if spot <= 0:
            continue
        if pc <= 0 or abs(pc - canonical) / canonical > _PREV_CLOSE_TOL:
            r["prev_close"] = round(canonical, 2)
            r["change_pct"] = round((spot - canonical) / canonical * 100.0, 3)
            changed = True
    return rows, changed
...
def _signal_nifty_indicative(quote: Optional[Dict[str, Any]],
                             canonical_prev: Optional[float] = None) -> Dict[str, Any]:
    ...
    if canonical_prev and canonical_prev > 0:
        # Provider feed occasionally returns a recent indicative open as
        # prev_close; trust the daily-history anchor when it diverges.
        if prev <= 0 or abs(prev - canonical_prev) / canonical_prev > _PREV_CLOSE_TOL:
            prev = canonical_prev
    if spot <= 0 or prev <= 0:
...
def _compute_pulse() -> Dict[str, Any]:
    ...
    sig_nifty = _signal_nifty_indicative(
        quotes.get(NIFTY_INDEX),
        canonical_prev=_canonical_prev_close(now.strftime("%Y%m%d")),
    )
    ...
    # Back-fix any points written with a glitched prev_close before this
    # session's anchor was established.
    canonical_prev = sig_nifty.get("prev_close") if sig_nifty.get("available") else None
    if not canonical_prev:
        canonical_prev = _canonical_prev_close(day_key)
    series, _fixed = _sanitize_series(series, canonical_prev)
    if _fixed:
        shared_cache.jset(series_key, series, ttl=_SERIES_TTL)
