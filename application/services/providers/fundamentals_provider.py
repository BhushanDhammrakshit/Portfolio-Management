...
def get_fundamentals(symbol: str, *, force: bool = False) -> dict:
    ...
    Persisted in Table Storage until an explicit refresh. Pass ``force=True``
    to re-fetch from yfinance and overwrite the stored snapshot.
    """
    if not symbol:
        return _empty_payload("")
    from application.services import snapshot_store
    return snapshot_store.serve_or_refresh(
        CACHE_PREFIX + symbol.upper(),
        lambda: _build_fundamentals(symbol),
